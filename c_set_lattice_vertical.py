...
# Dibujar el lattice usando niveles jerárquicos y rotarlo
pos = nx.multipartite_layout(G, subset_key="subset_key")

# Intercambiar las coordenadas x e y para rotar el layout
...
nx.draw(
    G, rotated_pos, with_labels=True, labels=labels, node_size=3000, node_color="skyblue", 
    font_size=8, font_color="black", font_weight="bold", edge_color="gray", arrows=True
)

# Añadir título
plt.title("Látice de Conceptos (Rotado Verticalmente)", fontsize=16)
plt.savefig('img/c_set_lattice_vertical.png')
plt.tight_layout()
plt.show()
